...
def handle_deploy():
    ...
    3. Sends request to Management Plane to create the deployment
    """
    ...
    for file in files:
        ...
        # split and remove the top-level folder
        parts = Path(raw_filename).parts  # handles slashes correctly across OSes
        parts_without_parent = parts[1:]  # remove top level folder

        # sanity check: ensure no traversal
        for part in parts_without_parent:
            if part in ("", ".", ".."):
                abort(400, f"Invalid path part: {part}")

        # rebuild safe relative path
        safe_rel_path = Path(*parts_without_parent)

        # ensure full path is inside upload folder
        save_path = Path(upload_subfolder) / safe_rel_path
    ...
    with zipfile.ZipFile(zip_path, "w") as zipf:
        for root, dirs, files_in_dir in os.walk(upload_subfolder):
            for fname in files_in_dir:
                ...
                zipf.write(file_path, arcname)  # preserves folder structure inside zip

    # make bucket if not found
    found = min_client.bucket_exists(DEPLOY_BUCKET)
    if not found:
